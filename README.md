...
### Top-level discriminators with @Discriminant

When producing code for environments that require flat, single-token top-level types (such as TypeScript API bindings generated by Dollup), the discriminator enumeration must be declared directly at file scope rather than nested within a namespace. However, Swift forbids peer macros from introducing arbitrary names at global scope.

The `@Discriminant` macro provides an inverted architecture: you declare the outer discriminator enumeration at the top level and nest the payload-bearing variants in an inner enum. The macro automatically detects the solitary nested enum, populates the outer cases, and synthesizes a mapping initializer:
...
Call sites obtain the discriminator tag from a variant value using standard conversion syntax:

```swift
let value: TooltipType.Union = .text("Hello")
let type: TooltipType = .init(value) // .text
```

If the enclosing type contains multiple nested enumerations, you can specify the target enum explicitly using `@Discriminant(of: "...")`. Raw backing types (such as `enum TooltipType: String`) are fully supported by applying the raw type directly to the host enum declaration.
